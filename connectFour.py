...
def winner(board,player):
    ...
    for row in range(ROW_COUNT-3):
        for col in range(COLUMN_COUNT):
            if board[row][COLUMN_COUNT-1] == player and board[row+1][COLUMN_COUNT-2] == player and board[row+2][COLUMN_COUNT-3] == player and board[row+3][COLUMN_COUNT-4] == player :
                return True
    return False
